...
export default function UserCard({ user, appRole }) {
  return (
    <div className="bg-gradient-to-br from-primary-50 to-primary-50 border border-primary-200 rounded-lg p-3 shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-medium text-gray-700 uppercase tracking-wide">User Info</p>
          <p className="text-sm font-bold text-gray-900 mt-1 truncate">{user?.full_name || user?.name || 'User'}</p>
          <p className="text-xs text-gray-700 truncate">{user?.username || 'username'}</p>
          <div className="flex gap-2 items-center mt-2">
            <span className={`inline-block px-2 py-0.5 rounded-full text-white text-xs font-bold ${
              appRole === 'admin' ? 'bg-info' : 'bg-primary-600'
            }`}>
