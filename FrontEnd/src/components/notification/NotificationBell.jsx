...
export default function NotificationBell() {
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={toggle} className="relative p-2 rounded hover:bg-primary-50">
        <Bell size={20} className="text-primary-700" />
        {unreadCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 inline-flex items-center justify-center px-1.5 py-0.5 text-xs font-bold leading-none text-white bg-primary-600 rounded-full">{unreadCount}</span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 bg-white border rounded shadow-lg z-50">
          <div className="p-3 border-b flex items-center justify-between">
            <div className="text-sm font-medium text-primary-900">Notifikasi</div>
            <button onClick={markAllRead} className="text-xs text-primary-600">Tandai semua dibaca</button>
          </div>
          ...
          <div className="max-h-64 overflow-auto">
            {items.length === 0 ? (
              ...
              items.map((it) => (
                <div key={it.id} className={`p-3 border-b ${it.read ? 'bg-white' : 'bg-primary-50'}`}>
                  <div className="text-sm text-primary-800">{it.title}</div>
