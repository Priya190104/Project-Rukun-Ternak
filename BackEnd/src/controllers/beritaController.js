...
const getAllBerita = async (req, res) => {
  try {
    ...
    const { rows } = await db.query('SELECT id, caption, image_url AS "imageUrl", created_at AS "createdAt", updated_at AS "updatedAt" FROM berita ORDER BY created_at DESC');
...
const getBeritaById = async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await db.query('SELECT id, caption, image_url AS "imageUrl", created_at AS "createdAt", updated_at AS "updatedAt" FROM berita WHERE id=$1', [parseInt(id,10)]);
    const berita = rows[0];
    if (!berita) {
...
const createBerita = async (req, res) => {
  try {
    ...
    console.log('[Berita] CREATE request:', { caption: !!caption, imageUrl: !!imageUrl, bodyKeys: Object.keys(req.body) });
    ...
    console.log('[Berita] Inserting to database:', { caption, imageUrl });
    const { rows } = await db.query('INSERT INTO berita (caption, image_url, created_at, updated_at) VALUES ($1, $2, NOW(), NOW()) RETURNING id, caption, image_url AS "imageUrl", created_at AS "createdAt", updated_at AS "updatedAt"', [caption, imageUrl]);
    const berita = rows[0];
    const processedBerita = processBeritaData(berita);
...
const updateBerita = async (req, res) => {
  try {
    ...
    console.log('[Berita] UPDATE request:', { id, caption: !!caption, imageUrl: !!imageUrl });
    ...
    if (imageUrl) {
      ...
      query = 'UPDATE berita SET caption=$1, image_url=$2, updated_at=NOW() WHERE id=$3 RETURNING id, caption, image_url AS "imageUrl", created_at AS "createdAt", updated_at AS "updatedAt"';
      params = [caption, imageUrl, parseInt(id, 10)];
    } else {
      // Update only caption
      query = 'UPDATE berita SET caption=$1, updated_at=NOW() WHERE id=$2 RETURNING id, caption, image_url AS "imageUrl", created_at AS "createdAt", updated_at AS "updatedAt"';
      params = [caption, parseInt(id, 10)];
    }
